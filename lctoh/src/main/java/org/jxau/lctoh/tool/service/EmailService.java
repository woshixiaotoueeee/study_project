...
public class EmailService {
	...
	public void putprops() throws GeneralSecurityException{
		props.setProperty("mail.transport.protocol", EmailConfig.mailTransportProtocol);   // 使用的协议（JavaMail规范要求）
	    props.setProperty("mail.smtp.host", EmailConfig.myEmailSMTPHost);   // 发件人的邮箱的 SMTP 服务器地址
	    props.setProperty("mail.smtp.auth", EmailConfig.mailSmtpAuth);  // 需要请求认证 
	    props.setProperty("mail.auth", EmailConfig.mailAuth);
	    
	    MailSSLSocketFactory sf = new MailSSLSocketFactory();
	    sf.setTrustAllHosts(true);
	    props.put("mail.smtp.ssl.enable",EmailConfig.mailSmtpSslEnable);
	...
	public void sendEmail(String receiveMailAccount,String popTile,String sendMSGTile,String sendMSGInfo) throws Exception{
	    ...
	    session.setDebug(EmailConfig.debug); // 设置为debug模式, 可以查看详细的发送 log
        // 3. 创建一封邮件
        MimeMessage message = createMimeMessage(session, EmailConfig.myEmailAccount, receiveMailAccount, popTile, sendMSGTile, sendMSGInfo);
        // 4. 根据 Session 获取邮件传输对象
        Transport transport = session.getTransport();

        transport.connect(EmailConfig.myEmailAccount, EmailConfig.myEmailPassword);
    ...
    private static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail,String popTile,String sendMSGTile,String sendMSGInfo) throws Exception {
        ...
        message.setFrom(new InternetAddress(sendMail, EmailConfig.sendTile, EncodingConfig.characterEncoding));
